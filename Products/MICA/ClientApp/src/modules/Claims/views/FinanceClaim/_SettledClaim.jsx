...
import Settledclaims from "assets/img/Settled-claims.png";
...
class PaymentFailure extends React.Component {
    ...
    dataTable = () => {
        ...
        this.setState({
            tabledata: this.state.setteled.map((prop, key) => {
                var i = true;
        ...
        for (let i = 0; i < this.state.setteled.length; i++) {
            this.state.claimid.push(this.state.setteled[i].claimId);
        }
        let claim = this.state.claims;
        claim.ClaimId = [...this.state.claimid];
        this.setState({ claim });
        console.log("claimid array: ", this.state.claims);
        console.log("claimid array: ", this.state.setteled);
    ...
    handlesearch = () => {
        ...
        fetch(`${ClaimConfig.claimConfigUrl}/api/ClaimManagement/GetSettledFinanceData`, {
            method: 'Post',
            headers: {
            ...
            .then(data => {
                this.setState({ setteled: data });
                console.log("response data ", data)
                this.setState({ showtable: false, loader: false });
                if (this.state.setteled.length > 0) {
                    this.dataTable();
                } else {
                    setTimeout(
                        function () {
                ...
                {this.state.pageloader ?
                    ...
                    <Card>
                        <CardHeader color="rose" icon>
                            <CardIcon color="rose">
                                <Icon><img id="icon" src={Settledclaims} /></Icon>
                            </CardIcon>
                            <h4 className={this.props.cardIconTitle}>
                                <small><TranslationContainer translationKey="Settledclaims" /></small>
                            </h4>
                        </CardHeader>
                        <CardBody>
                            <GridContainer>
                ...
                {this.state.loader ?
                    <GridContainer xl={12}>
                        {this.state.showtable ?
                                ...
                                <Animated animationIn="fadeIn" animationOut="fadeOut" isVisible={true}>
                                <ReactTable
                                        title={<h5><TranslationContainer translationKey="SettledClaims" /></h5>}
