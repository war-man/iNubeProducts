...
class Partner extends React.Component {
    ...
    SetValue = (Type, event) => {
        debugger;
        console.log("list", this.state.indLst, this.state.corpLst)
        let PartnerDTO = this.state.PartnerDTO;
        let name = event.target.name;
        let value = event.target.value;

        if (this.state.PartnerDTO.partnerTypeId == 8) {
           // var listData = this.state.indLst;
            //this.setState({ mList: this.state.indLst });
            this.state.mList = this.state.indLst;
            this.setState({});
            //this.state.indLst.push(this.state.mList);
            //this.state.mList = this.state.indLst;
        }
        if (this.state.PartnerDTO.partnerTypeId == 9) {
            this.state.mList = this.state.corpLst;
            this.setState({});
        }
        this.setState({});
        console.log("mlist", this.state.mList, this.state.indLst, this.state.corpLst);
        ...
        if (name === "partnerCode") {
            ...
            fetch(`${partnerconfig.partnerconfigUrl}/api/Partner/PartnerCodevalidation?partnercode=` + value, {
                ...
                .then(data => {
                    ...
                    if (data.status === 9) {
                        var msg = CommonMessage("PartnerCodeMsg", []);
    ...
    componentDidMount() {
        debugger;
        console.log("pdata", this.state.PartnerDTO);
        fetch(`${partnerconfig.partnerconfigUrl}/api/Organization/GetMasterData?sMasterlist=OrgCategory`, {
          ...
          .then(data => {
              ...
              const salList = (this.state.masterList.filter(e => e.mType === 'Salutation')[0]) === undefined
                  ? []
                  : this.state.masterList.filter((e) => e.mType === 'Salutation')[0].mdata;
              var arrlength = salList[salList.length - 1];
             this.state.corpLst.push(arrlength);
              var arr = salList.slice(0,salList.length - 2);
              this.setState({ indLst: arr });
              console.log("salutation", arrlength, arr)
          });
