...
namespace iNube.Edelweiss.BalanceAlert.AwsLambda
{
    public class Function
    {

        /// <summary>
        /// A simple function that Calls Extension Module to alert AD Balance is Zero
        ...
        public async Task<dynamic> FunctionHandler(ILambdaContext context)
        {
            ...
            client.BaseAddress = new Uri("https://egiswitchapi.edelweissinsurance.com/api/Mica_EGI/BalanceAlertScheduler");
                ...
                using (var content = response.Content)
                {
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        if (!String.IsNullOrEmpty(serviceResponse))
                        {
                            ...
                            {
                                Message = $"AD Balance Check Notification Scheduler successfully executed & Its Status - " + serviceResponse,
                        ...
                        else
                        {
                            var request = new PublishRequest
                            {
                                Message = $"AD Balance Check Notification Scheduler failed - reponse did not come as expected",
                                TopicArn = "arn:aws:sns:ap-south-1:471379395009:SchedulerTopic",
                            };
            ...
            catch
            {
                var request = new PublishRequest
                {
                    Message = $"AD Balance Check Notification Scheduler failed due to some exception",
                    TopicArn = "arn:aws:sns:ap-south-1:471379395009:SchedulerTopic",
                };
