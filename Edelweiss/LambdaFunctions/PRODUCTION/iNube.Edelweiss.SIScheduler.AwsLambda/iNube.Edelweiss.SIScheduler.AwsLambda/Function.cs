...
namespace iNube.Edelweiss.SIScheduler.AwsLambda
{
    public class Function
    {

        /// <summary>
        /// A simple function that Calls MICA Extension Module Calculate Monthly Standing Instruction API
        ...
        public async Task<dynamic> FunctionHandler(DateTime? dateTime, ILambdaContext context)
        {
            ...
            if (dateTime == null)
            {
                client.BaseAddress = new Uri("https://egiswitchapi.edelweissinsurance.com/api/Mica_EGI/MonthlySIScheduler");
            }
            else
            {
                client.BaseAddress = new Uri("https://egiswitchapi.edelweissinsurance.com/api/Mica_EGI/MonthlySIScheduler?dateTime=" + dateTime);
            }
                ...
                using (var content = response.Content)
                {
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        if (!String.IsNullOrEmpty(serviceResponse))
                        {
                            ...
                            {
                                Message = $"Monthly SI Scheduler successfully executed & Its Status - " + serviceResponse,
                        ...
                        else
                        {
                            var request = new PublishRequest
                            {
                                Message = $"Monthly SI Scheduler failed - reponse did not come as expected",
                                TopicArn = "arn:aws:sns:ap-south-1:471379395009:SchedulerTopic",
                            };
            ...
            catch
            {
                var request = new PublishRequest
                {
                    Message = $"Monthly SI Scheduler failed due to some exception",
                    TopicArn = "arn:aws:sns:ap-south-1:471379395009:SchedulerTopic",
                };
