...
namespace iNube.Edelweiss.Cancel.AwsLambda
{
    public class Function
    {
        /// <summary>
        /// A simple function that Calls Policy Module to Cancel all the Proposal On 15th Day
        ...
        public async Task<dynamic> FunctionHandler(DateTime? dateTime, ILambdaContext context)
        {
            ...
            if (dateTime == null)
            {
                client.BaseAddress = new Uri("https://egiswitchapi.edelweissinsurance.com/api/Policy/ProposalCancellationScheduler");
            }
            else
            {
                //client.BaseAddress = new Uri("https://egiswitchapi.edelweissinsurance.com/api/Policy/ProposalCancellationScheduler?dateTime=" + dateTime);
                return false;
            }
                ...
                using (var content = response.Content)
                {
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        if (!String.IsNullOrEmpty(serviceResponse))
                        {
                            ...
                            {
                                Message = $"Proposal Cancelation Scheduler successfully executed & Its Status - " + serviceResponse,
                        ...
                        else
                        {
                            var request = new PublishRequest
                            {
                                Message = $"Proposal cancellation scheduler failed - reponse did not come as expected",
                                TopicArn = "arn:aws:sns:ap-south-1:471379395009:SchedulerTopic",
                            };
            ...
            catch
            {
                var request = new PublishRequest
                {
                    Message = $"Proposal cancellation scheduler failed due to some exception",
                    TopicArn = "arn:aws:sns:ap-south-1:471379395009:SchedulerTopic",
                };
