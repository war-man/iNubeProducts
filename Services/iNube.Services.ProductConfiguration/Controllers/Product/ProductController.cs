...
#pragma warning restore CS1998 // Async method lacks 'await' operators and will run synchronously
        ...
        [HttpPost]
        public async Task<IActionResult> SaveEntities(EntityDetailsDTO entityDetails)
        {
            var response = await _productService.SaveEntities(entityDetails, Context);
            return Ok(response);
        }
        ...
        [HttpGet]
        public async Task<IActionResult> GetEntities(int parentid)
        {
            var response = await _productService.GetEntities(parentid, Context);
            return Ok(response);
        }
