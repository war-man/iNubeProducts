...
{
    public class MicaLoginService : ILoginProductService
    {
        ...
        public MicaLoginService(IMapper mapper, IOptions<AppSettings> appSettings, IConfiguration configuration, IEmailService emailService)
        {
            _mapper = mapper;
        ...
        public LoginResponse GenerateToken(AspNetUsersDTO user, string productType, decimal envId, bool isTokenExpire)
        {
            ...
            if (isTokenExpire == true)
            {
                ...
                var expiry = isTokenExpire ? DateTime.Now.AddYears(3) : DateTime.Now.AddYears(3);
